  ...
  update(dt, p) {
    ...
    if (this.mode === 'auto' && !this.shifting && controlsActive) {
      if (absVF < 0.6) {
        ...
        const wantUp = rpmNow > T.autoUpshiftRpm && this.gear < 6;
        ...
        if (wantUp || wantDown || kick) {
          const next = wantUp ? this.gear + 1 : this.gear - 1;
          ...
          if (rpmNext < T.rpmMaxSafe) {
            ...
            this.shiftCooldown = wantUp ? 0.5 : 0.35;
